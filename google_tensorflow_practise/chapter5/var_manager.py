...
def inference(input_tensor, reuse, avg_class=None):
    """
    定义网络结构，返回前向传播的结果
    ...
    if avg_class == None:
        ...
        with tf.variable_scope("layer1", reuse=reuse):
            weigths = tf.get_variable("weigths", shape=[INPUT_NODE, LAYER1_NODE],
        ...
        with tf.variable_scope("layer2", reuse=reuse):
            weigths = tf.get_variable("weigths", shape=[LAYER1_NODE, OUTPUT_NODE],
                                      initializer=tf.truncated_normal_initializer(0.1))
        ...
        with tf.variable_scope("layer1", reuse=reuse):
            ...
            biases = tf.get_variable("biases", shape=[LAYER1_NODE],
                                     initializer=tf.constant_initializer(0.1))
            layer1 = tf.nn.relu(tf.matmul(input_tensor, avg_class.average(weigths)
        ...
        with tf.variable_scope("layer2", reuse=reuse):
            ...
            biases = tf.get_variable("biases", shape=[OUTPUT_NODE],
                                     initializer=tf.constant_initializer(0.1))
            layer2 = tf.matmul(layer1, avg_class.average(weigths)
                               + avg_class.average(biases))
        return layer2
...
def train(mnist):
    ...
    # 计算使用了滑动平均的时候的前向传播
    y_average = inference(x, True, variable_averages)
    ...
    with tf.variable_scope("", reuse=True):
        regularization = regularizer(tf.get_variable("layer1/weigths"
                                                     , shape=[INPUT_NODE, LAYER1_NODE])) + \
                         regularizer(tf.get_variable("layer2/weigths"
    ...
    # 检验使用滑动平均模型的神经网络是否正确。
    correct_prediction = tf.equal(tf.argmax(y_average, 1), tf.argmax(y_label, 1))
    # cast将bool数值变成实数
    ...
    with tf.Session() as sess:
        ...
        for i in range(TRAINING_STEPS):
            ...
            if i % 1000 == 0:
                ...
                print("After %d training step(s),validation accurary using "
                      "average model is %g, test accurary is %g"
